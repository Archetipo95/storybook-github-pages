...
jobs:
  deploy-storybook:
    uses: Archetipo95/storybook-github-pages/.github/workflows/deploy-storybook.yml@v1.9.6
    with:
  ...
  build-and-deploy:
    ...
    steps:
      ...
      - name: Build and Deploy Storybook
        uses: Archetipo95/storybook-github-pages@v1.9.6
        with:
          path: 'storybook-static'
          build_command: 'npm run build-storybook'
```
  ...
  publish:
    ...
    steps:
      ...
      - name: Publish directory
        uses: Archetipo95/storybook-github-pages/publisher@v1.9.6
        with:
...
| **Tagging Strategy** | Immutable release tags (for example, `@v1.9.6`)                                          | **Recommended for stable, reproducible use.** Floating major tags (e.g. `@v1`) are optional and non-reproducible. |
...
Simply replace `bitovi/github-actions-storybook-to-github-pages@v1.0.3` with `Archetipo95/storybook-github-pages@v1.9.6` in your workflow.
...
- [ ] Confirm CI is green in this repository.
- [ ] Run the `storybook-vue-demo` Action Canary against the candidate branch, tag, or SHA.
- [ ] Cut and tag the release.
- [ ] Update the demo repository's stable refs after the tag exists.
