...
test('createDeployment creates a pending deployment and status record', async () => {
  ...
  global.fetch = async (url, options = {}) => {
    ...
    if (url === 'https://api.github.com/repos/acme/app/deployments/77/statuses' && (options.method || 'GET') === 'POST') {
      return {
        ok: true,
        status: 201,
      ...
      return {
        ...
        text: async () =>
          JSON.stringify([
            ...
            { id: 22, environment: 'pr-preview', description: 'Storybook preview for PR #42', payload: { pr_number: 42 } },
            { id: 33, environment: 'pr-preview', description: 'Other deployment' }
          ])
      };
    }

    if (url === 'https://api.github.com/repos/acme/app/deployments/22/statuses' && (options.method || 'GET') === 'POST') {
      return {
        ok: true,
        status: 201,
