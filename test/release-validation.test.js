...
test('release validation - package.json runtime dependency cleanliness', () => {
  ...
  assert.equal(pkg.version, '1.9.6', 'package.json version must be set to 1.9.6 for stable release');
  assert.ok(
...
test('release validation - immutable release tag recommended in README and issue templates', () => {
  ...
  assert.match(
    readme,
    /uses:\s*Archetipo95\/storybook-github-pages\/\.github\/workflows\/deploy-storybook\.yml@v1\.9\.6/,
    'README reusable workflow example must use immutable release tag @v1.9.6'
  );
  assert.match(
    readme,
    /uses:\s*Archetipo95\/storybook-github-pages@v1\.9\.6/,
    'README composite action example must use immutable release tag @v1.9.6'
  );
  assert.match(
    readme,
    /replace `bitovi\/github-actions-storybook-to-github-pages@v1\.0\.3` with `Archetipo95\/storybook-github-pages@v1\.9\.6`/,
    'README migration guide must specify immutable release tag @v1.9.6'
  );
  assert.match(
    readme,
    /Immutable release tags \(for example, `@v1\.9\.6`\)/,
    'README support matrix must recommend current immutable release tags'
  );
  assert.match(
    bugReport,
    /uses:\s*Archetipo95\/storybook-github-pages@v1\.9\.6/,
    'Bug report template must use immutable release tag @v1.9.6'
  );
});
...
test('release validation - directory mode integration documents dedicated publisher action', () => {
  ...
  assert.match(
    readme,
    /uses:\s*Archetipo95\/storybook-github-pages\/publisher@v1\.9\.6/,
    'README Option 3 directory mode pipeline must use publisher@v1.9.6'
  );
  assert.match(
    readme,
